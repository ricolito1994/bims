...
class SocketServerPHP {
		...
		if (!is_string($obj)){
			...
			//var_dump($msg);
				...
				foreach ( $search_arr as $users_index ){
					#echo $users_index . "\n";
					#var_dump($socket_users_client);
					#echo "\n\n";
					#echo $users_index;
					#echo "\n\n\n";
					$found = $this->search_array_recursion( $socket_users_client , $users_index , 1 );
					#echo $users_index."\n\n";
					#var_dump($found );
					#var_dump($users_index);
					#echo "\n";
					$sockets_to_be_send[] = $found;
					
				}
				#echo "\n\n";
				#print_r($sockets_to_be_send);
	...
	public function addCompanyRoom ( $room_name ){
		$this->socket_rooms_company[$room_name] = array();
	}
	
	/* adds a server company room */	
	public function setCompanyRoomApp ( $room_name , $application ){
		$this->socket_rooms_company[ $room_name ][ $application ] = array();
	}
