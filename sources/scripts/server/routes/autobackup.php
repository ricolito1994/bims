...
$MAINTENANCE_SETTING=$LOCAL_DATABASE->execute( "SELECT * FROM bis.maintenance LIMIT 1" ,[] , \PDO :: FETCH_ASSOC )->fetch();
...
$LOCAL_DATABASE->execute( "UPDATE bis.maintenance SET IS_AUTO_BACK_UP_RUNNING = 1" ,[] , \PDO :: FETCH_ASSOC );
 */
...
while (true){
	...
	if($alive_once){
		$RES=$LOCAL_DATABASE->execute( "SELECT 'HI IM STILL ALIVE PLEASE DONT TERMINATE ME!' as MSG FROM bis.company_setup LIMIT 1" ,[] , \PDO :: FETCH_ASSOC );
