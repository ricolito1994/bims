...
export class GoogleMapsAPI {
		...
		if (!this.promise) {
		  this.promise = new Promise(resolve => {
			...
			if (typeof window.google === 'undefined') {
			  ...
			  script.src = `https://maps.googleapis.com/maps/api/js?key=${this.API_KEY}&callback=${this.callbackName}&libraries=places&v=beta`;
			  script.async = true;
	...
	loadLocations (  ){
		let load = new LoadingModal ({
			...
			template : `/bis/sources/templates/modal/loading.modal.template.html`,
			...
			/* const featureStyleOptions = {
					...
					fillOpacity: 0.5,
					  ...
					  featureLayer.style = (options) => {
						if (options.feature.placeId == "ChIJW3hh3q7RrjMRa_eJG1Ryeeo") {
	...
	loadMapsCallback (){
		this.load().then(() => {
		   ...
		   var mapOptions= {
			...
			zoom:this.GOOGLE_MAPS_OBJECT.ZOOM, 
			scrollwheel: this.GOOGLE_MAPS_OBJECT.SCROLLWHEEL,
			draggable: true,
			//mapTypeId:google.maps.MapTypeId.ROADMAP
			 mapTypeId: this.GOOGLE_MAPS_OBJECT.MAP_TYPE,
			 mapId : this.GOOGLE_MAPS_OBJECT.MAP_ID,
		  };
		 // console.log(this.GOOGLE_MAPS_OBJECT.MAP_ID)
		  let map = new google.maps.Map(document.querySelector('#maps-container'),mapOptions);
		  
		  this.map = map;
		  this.map.setTilt(this.GOOGLE_MAPS_OBJECT.TILT);
	...
	newLocation( event ){
		try{
			let markerLocs = GoogleMapsAPI.getMyMarker({
				...
				label : `<div style="font-size:100%;text-align:center">${event.ICN}</div>`,
			});
