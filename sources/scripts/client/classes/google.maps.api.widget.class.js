...
export class GoogleMapsAPIWidget extends Modal{
	...
	loadLocations ( ){
		...
		return new Promise ( ( resolve , reject ) => {
			...
			let DefaultLocation = {
				...
				VALUE : {
					content : 
						`<div style="font-size:20px;text-align:center"><img width="40" src="/bis/sources/complist/mansilingan_bcd/logos/${session_data.BARANGAY_LOGO}"></div>
	...
	newLocation ( ...args ){
		...
		let nl = new NewLocationModalController({
			...
			template : "/bis/sources/templates/modal/new.location.modal.template.html",
			parent : this,
			args : {
				close : "onCloseAddLoc",
				save : "onSaveLoc",
