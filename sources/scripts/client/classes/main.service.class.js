...
export class MainService {
	...
	constructor ( rootDirectory , sessionData ){
		...
		this.tinyMceOptions = {
            ...
            plugins: `preview powerpaste casechange searchreplace autolink autosave save directionality advcode 
				      visualblocks visualchars fullscreen image link media mediaembed template codesample advtable table 
				      charmap pagebreak nonbreaking anchor advlist lists checklist wordcount tinymcespellchecker a11ychecker
					  help formatpainter permanentpen pageembed linkchecker emoticons export hr`,
			toolbar: `undo redo | image code | casechange blocks | bold italic backcolor | hr |
		...
		this.documentBody = [
			{},
			{
				...
				body : `
				<h1> BARANGGAY CLEARANCE </h1>
				<p> <input id='doc-modal-issued-to-tiny-mce' 
				style='border-top-style: hidden;border-right-style: hidden;border-left-style: hidden;border-bottom-style: 
				groove;background-color: #eee;' value="ISSUED_TO_NAME"  type='text'>
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
				Fusce sed ex in dolor laoreet cursus. Nulla cursus nunc ac scelerisque
				fringilla. Aliquam a ornare mauris. Maecenas id odio at massa posuere
				porttitor a et quam. Praesent nisl mi, hendrerit at consequat quis, 
				mollis ac dui. Cras laoreet tincidunt imperdiet. Praesent non libero 
				vitae dui condimentum molestie nec ut felis. Maecenas eget eros eget 
				tortor accumsan dictum. Suspendisse facilisis arcu dictum urna semper,
				eget iaculis elit ultrices. Proin efficitur scelerisque rhoncus.</p>
				<h3>ISSUED BY</h3> <input id='doc-modal-issued-to-tiny-mce' 
				style='border-top-style: hidden;border-right-style: hidden;border-left-style: hidden;border-bottom-style: 
				groove;background-color: #eee;' value="ISSUED_BY_NAME"  type='text'><BR>
				<h3>DATE</h3> <input id='doc-modal-issued-to-tiny-mce' 
				style='border-top-style: hidden;border-right-style: hidden;border-left-style: hidden;border-bottom-style: 
				groove;background-color: #eee;' value="ISSUED_DATE"  type='text'>`,
				replace : [
					'ISSUED_TO_NAME',
					'ISSUED_BY_NAME',
					'ISSUED_DATE',
				],
				regex : /\b(?:ISSUED_TO_NAME|ISSUED_BY_NAME|ISSUED_DATE)\b/gi
			},
			{
	...
	async init () {
		...
		this.urls = {
			"auth" : { url : this.rootDirectory+'/scripts/server/routes/auth.route.server.php' },
			"generic" : { url : this.rootDirectory+'/scripts/server/routes/profile.route.server.php' },
			"event" : { url : this.rootDirectory+'/scripts/server/routes/profile.route.server.php' },
			"payroll" : { url : this.rootDirectory+'/scripts/server/routes/profile.route.server.php' },
		}

		let appname = await this.getConstant('APP_NAME');
			appname = appname.res;
		let sessionData = await this.getCurrentSessionData();
			sessionData = sessionData.res;
		...
		/* if ( sessionData )
		this.sessionData = sessionData; */
		this.sourcesDirectory = this.rootDirectory;
		this.listeners = [];
		this.isPropertyCustodian = sessionData ? this.sessionData.POSITION == "Property Custodian" : false;
		this.isAdministrator = sessionData ? this.sessionData.DESIGNATION == "System Administrator": false;
	}
	...
	static getIpAddress () {
		return new Promise ( ( resolve, reject) => {
			let request = {
				type: "POST",
				url : '/bims/sources/scripts/server/routes/profile.route.server.php',
				data : {
					data : {
			...
			let request1 = {
				...
				data : {
					data : {
						...
						REQUEST_QUERY : [
							{
								sql : "select * from bis.unit where ITEM_CODE = ? ORDER BY HEIRARCHY",
								db : 'DB',
								query_request : 'GET',
								index : 'units',
								values : [
							...
							{
								sql : "select * from bis.unit where ITEM_CODE = ? and UNIT_NAME = ?",
								db : 'DB',
								query_request : 'GET',
								index : 'my_unit',
								values : [
			...
			let request = {
						...
						data : {
							data : {
								...
								REQUEST_QUERY : [
									{
										sql : "SELECT * FROM bis.ITEM_MASTER_DATA WHERE ITEM_CODE = ?",
										db : 'DB',
										query_request : 'GET',
										index : 'ITEM',
										values : [ ITEM_CODE ]
									...
									{
										sql : "SELECT * FROM bis.UNIT WHERE ITEM_CODE = ?",
										db : 'DB',
										query_request : 'GET',
										index : 'UNIT',
										values : [ ITEM_CODE ]
	...
	#generateStockCardData( item, FRM, TO, WH ){
		return new Promise ( async ( resolve, reject ) => {
			...
			let SQL_STOCK_CARD = `SELECT 
					(SELECT WAREHOUSE_NAME FROM bis.warehouse WHERE WAREHOUSE_CODE = iiw.WAREHOUSE_CODE) WHNAME ,
					(SELECT WAREHOUSE_NAME FROM bis.warehouse WHERE WAREHOUSE_CODE = iiw.WAREHOUSE_CODE_FRM) WHNAMEFRM ,
					(SELECT WAREHOUSE_CODE FROM bis.warehouse WHERE WAREHOUSE_CODE = iiw.WAREHOUSE_CODE) WHCODE ,
					(SELECT WAREHOUSE_CODE FROM bis.warehouse WHERE WAREHOUSE_CODE = iiw.WAREHOUSE_CODE_FRM) WHCODEFRM ,
					(SELECT CONCAT(FIRSTNAME,' ',LASTNAME) FROM bis.user_setup WHERE ID = ad.TRANSFERED_TO) TRANSFER_TO_ ,
					(SELECT CONCAT(FIRSTNAME,' ',LASTNAME) FROM bis.user_setup WHERE ID = ad.TRANSFERED_FROM) TRANSFER_FRM_ ,
					(SELECT SUPPLIER_NAME FROM bis.supplier WHERE SUPPLIER_CODE = ad.SUPPLIER_ID) SUPPLIER_ ,
					...
					id.EXPIRY_DATE AS EXPIRY_DATE from bis.alter_document ad 
					 inner join bis.item_details id ON id.DOC_ID = ad.DOC_ID 
					 INNER JOIN bis.item_master_data imd ON id.ITEM_CODE = imd.ITEM_CODE 
					 inner join bis.unit u ON u.UNIT_CONVERSION_ID = id.UNIT_CONVERSION_ID 
					 left join ( 
						select 
						ID, 
						EXPIRY_DATE,
				...
				let SQL_BEG_BAL = `SELECT * FROM bis.item_in_warehouse iiw 
	...
	static SR ( serverRequestObject , thenCallback , catchCallback , async ){
	...
	getBrgyEmployee ( id ) {
		let sql = `SELECT emp.ID, emp.RESIDENT_ID, emp.IS_BRGY_CAPTAIN, CONCAT(res.FIRSTNAME,' ',res.LASTNAME) AS FNAME FROM bis.barangay_emp_setup emp 
				   INNER JOIN bis.barangay_res_setup res ON res.RESIDENT_ID = emp.RESIDENT_ID WHERE emp.ID = ?`;
		return new Promise ( ( resolve, reject) => {
			let request = {
				type: "POST",
				url : this.urls["generic"].url,
