...
export class ResidentsSectionController extends Modal {
	...
	constructs(){
		this.dataTable = new DataTableService({
			template : `/${this.mainService.appname}/sources/templates/section/datatable.template.section.html`,
			controller : this,
							...
							attributes : [
								{
									...
									value : ( selData ) => {
										...
										return `/${this.mainService.appname}/sources/images/prof.jpg`;
										//else
										//return '';
									},
								},
										...
										let dataQuery = {
											...
											data : {
												data : {
													...
													REQUEST_QUERY : [
														{
															sql : `select  RS.FULLNAME,RS.RESIDENT_ID,RS.BIRTHDAY from ${this.mainService.appname}.barangay_res_setup RS
																	where RS.HH_LEADER = ?`,
															db : 'DB',
															index : 'hh_members',
															query_request : "GET",
	...
	openResidentModal( args ){
		let nl = new ResidentsModalController({
			...
			template : `/${this.mainService.appname}/sources/templates/modal/residents.modal.template.html`,
			parent : this,
		...
		let dataQuery = {
			...
			data : {
				data : {
					...
					REQUEST_QUERY : [
						{
							sql : `SELECT * FROM ${this.mainService.appname}.barangay_res_setup w WHERE CONCAT(LASTNAME,' ',FIRSTNAME,' ',MIDDLENAME) LIKE ? and w.ADDRESS like ?`+
