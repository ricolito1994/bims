...
export class ResidentsModalController extends Modal {
	...
	searchResident( arg ){
		...
		let ssm = new SearchModal ({
			...
			template : `/${this.mainService.appname}/sources/templates/modal/search.modal.template.html`,
			params : {
	...
	save(){
		...
		for ( let rhh in this.removedHHMembers ){
			...
			sqlforremovedmems += `UPDATE ${this.mainService.appname}.barangay_res_setup SET  HH_LEADER = ? WHERE RESIDENT_ID = ?;`;
		...
		for ( let mm in this.houseHoldMembers ){
			...
			sqlformres += `UPDATE ${this.mainService.appname}.barangay_res_setup SET PUROK = ? , HH_LEADER = ?, ADDRESS = ? WHERE RESIDENT_ID = ?;`;
