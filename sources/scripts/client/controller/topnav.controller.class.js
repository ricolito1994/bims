...
export class TopnavController extends Controller{
	...
	constructor ( controller , service , elem ){
		...
		this[controller] = {
			...
			companylogo : `/${this.mainService.appname}/sources/complist/${session_data.BARANGAY_DIR}/logos/${session_data.BARANGAY_LOGO}`,
	...
	switchDesignation(){
		let s = new SwitchDesignationModal ({
			...
			template : `/${this.mainService.appname}/sources/templates/modal/switch.account.modal.template.html`,
			parent : this,
		});
	...
	route ( params ){
		...
		window.history.pushState("", "", '/'+this.mainService.appname+'/?'+params.path);
		// window.history.pushState("", "", `/${this.mainService.appname}/?'${params.path}`);
