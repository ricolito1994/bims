...
export class ResidentsPurokController extends Modal {
	...
	openPurok(args){
		...
		let usm = new PurokModalController({
			...
			template : `/${this.mainService.appname}/sources/templates/modal/purok.modal.template.html`,
			parent : this,
					...
					REQUEST_QUERY : [
						{
							sql : `SELECT *,IF(CONCAT(RES.FIRSTNAME,' ',RES.LASTNAME) IS NOT NULL , CONCAT(RES.FIRSTNAME,' ',RES.LASTNAME) , '-') PRK_LEADER_NAME FROM 
									bis.barangay_prk_setup PRK LEFT JOIN bis.barangay_res_setup  RES ON RES.RESIDENT_ID = PRK.PRK_LEADER 
