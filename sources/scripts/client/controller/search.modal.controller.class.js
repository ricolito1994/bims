...
export class SearchModal extends Modal{
									...
									},	
									{
										...
										value : (...args )=> {
											...
											let usm = new ResidentsModalController({
												...
												template : "/bis/sources/templates/modal/residents.modal.template.2.html",
												parent : this,
													...
													MainService.EventObject[this.params.controller].dispatch (this.params.controller+this.params.evt , {
														detail : {
															query : {
																...
																AGE : this.mainService.calculateAge(arg['BIRTHDAY'].split("-"), this.mainService.getCurrentDate().split("-")),
																GENDER : arg['GENDER'] == 1 ? 'M' : 'F',
																AGENDER : arg['GENDER'],
																INC_ID : arg['ID'],
				...
				request : {
					...
					data : {
						data : {
							...
							REQUEST_QUERY : [
								{
									sql : `SELECT * FROM bis.barangay_res_setup WHERE FULLNAME LIKE ?  AND IS_FAMILY_LEADER = 0 order by ID desc`,
									query_request : 'GET',
									index : 'result',
									values : ['%%']
								},
				...
				request : {
					...
					data : {
						data : {
							...
							REQUEST_QUERY : [
								{
									sql : `SELECT *,(SELECT PRK_NAME FROM bis.barangay_prk_setup WHERE PRK_ID = PUROK) AS PRK_NAME FROM bis.barangay_res_setup WHERE FULLNAME LIKE ?  AND IS_FAMILY_LEADER = 1 order by ID desc`,
									query_request : 'GET',
									index : 'result',
									values : ['%%']
								},
									...
									},	
									{
										...
										value : (...args )=> {
											...
											let usm = new PurokModalController({
												...
												template : "/bis/sources/templates/modal/purok.modal.template.html",
												parent : this,
												isUpdate : args ? false : true,
												args : args?args : {},
												instanceID : this.mainService.generate_id_timestamp("res"),
							...
							REQUEST_QUERY : [
								{
									sql : `SELECT *,CONCAT(RES.FIRSTNAME,' ',RES.LASTNAME) PRK_LEADER FROM 
										bis.barangay_prk_setup PRK LEFT JOIN bis.barangay_res_setup  RES ON RES.RESIDENT_ID = PRK.PRK_LEADER WHERE PRK.PRK_NAME LIKE ?  order by PRK.ID desc`,
									query_request : 'GET',
									index : 'result',
									values : ['%%']
								},
	...
	constructs(){
		...
		this.dataTable = new DataTableService({
			template : "/bis/sources/templates/section/datatable.table.section.template.html",
			controller : this,
			controllername : "",
			tableID : "dttable",
