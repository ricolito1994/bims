...
require_once ("$root\scripts\server\constants\connection.server.constants.php");
$appname = APP_NAME;

$current_page = explode('/',$_SERVER['REQUEST_URI'])[2];
#echo $current_page == "barcodereader";
if (!isset ( $_SESSION ['PASSWORD'] ) && $current_page != 'login' ) header("Location: /$appname/login");
		

if (isset ( $_SESSION ['PASSWORD'] ) && $current_page == 'login') header("Location: /$appname");
...
?>
<title><?php ECHO isset($_SESSION['COMPANY_NAME']) ? $_SESSION['COMPANY_NAME'] : 'Barangay Information System 1.0';  ?></title>
<meta charset="utf-8">
...
<link rel="stylesheet" href="<?php echo $prefix;?>sources/styles/google.maps.btns.style.css<?php echo $ver; ?>" />
...
<script defer type="text/javascript" src="<?php echo $prefix;?>sources/imports/scripts/tinymce/tinymce.min.js<?php echo $ver; ?>" referrerpolicy="origin" ></script>
